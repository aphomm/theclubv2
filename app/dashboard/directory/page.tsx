    ...
    <div className="max-w-6xl">
      ...
      {isLoading ? (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="h-80 bg-stone-900 animate-pulse border border-stone-800" />
          ))}
        </div>
      ) : members.length === 0 ? (
              ...
              <div className="border border-stone-800 p-6 hover:border-amber-600 transition-colors cursor-pointer hover:bg-stone-900/50 h-full flex flex-col">
                ...
                <div className="flex gap-3 pt-4 border-t border-stone-800">
                  <Link
                    ...
                    className="flex-1 flex items-center justify-center gap-2 border border-stone-700 py-2 text-xs font-light text-stone-400 hover:border-amber-600 hover:text-amber-600 transition-colors"
                  >
                    <MessageCircle className="w-4 h-4" />
                    Message
                  </Link>
                  <button className="flex-1 border border-stone-700 py-2 text-xs font-light text-stone-400 hover:border-amber-600 hover:text-amber-600 transition-colors">
                    Connect
                  </button>
                </div>
              </div>
