    ...
    <div className="max-w-4xl">
      ...
      <div className="relative mb-8">
        ...
        <input
          ...
          autoFocus
          className="w-full bg-white/[0.04] border border-white/10 pl-12 pr-4 py-4 text-lg text-stone-100 placeholder:text-stone-600 focus:outline-none focus:border-amber-600 transition-colors rounded-full"
        />
      </div>
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-2">
          {tabs.map(tab => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 text-sm font-light border whitespace-nowrap transition-colors rounded-full ${
                activeTab === tab.id
      ...
      {isLoading ? (
        <div className="space-y-4">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="h-20 bg-white/[0.04] animate-pulse rounded-xl border border-white/[0.08]" />
          ))}
        </div>
      ) : !query ? (
        <div className="text-center py-20 rounded-2xl border border-white/[0.08]">
              ...
              <div className="space-y-2">
                {(activeTab === 'all' ? members.slice(0, 3) : members).map(member => (
                  <Link key={member.id} href={`/dashboard/directory/${member.id}`}>
                    <div className="rounded-xl border border-white/[0.08] p-4 hover:border-amber-600/60 transition-all hover:bg-white/[0.02] flex items-center gap-4">
                      ...
                      <span className="text-xs bg-amber-600/20 text-amber-600 px-2 py-1 font-light rounded-full">
                        {member.tier}
                      </span>
                    </div>
                  </Link>
              ...
              <div className="space-y-2">
                {(activeTab === 'all' ? events.slice(0, 3) : events).map(event => (
                  <Link key={event.id} href={`/dashboard/events/${event.id}`}>
                    <div className="rounded-xl border border-white/[0.08] p-4 hover:border-amber-600/60 transition-all hover:bg-white/[0.02]">
                      <div className="flex items-start justify-between">
                        ...
                        <span className="text-xs bg-amber-600/20 text-amber-600 px-2 py-1 font-light rounded-full uppercase">
                          {event.event_type}
                        </span>
                      </div>
                    </div>
                      ...
                      <div className="rounded-xl border border-white/[0.08] p-4 hover:border-amber-600/60 transition-all hover:bg-white/[0.02]">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            ...
                            <span className="text-xs bg-white/[0.08] text-stone-300 px-2 py-1 font-light rounded-full">
                              {project.category}
                            </span>
                          </div>
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          <div className="flex-1 bg-white/[0.08] h-1 rounded-full">
                            <div
              ...
              <div className="space-y-2">
                {(activeTab === 'all' ? resources.slice(0, 3) : resources).map(resource => (
                  <Link key={resource.id} href="/dashboard/resources">
                    <div className="rounded-xl border border-white/[0.08] p-4 hover:border-amber-600/60 transition-all hover:bg-white/[0.02] flex items-center justify-between">
